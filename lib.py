...
class genio:
    ...
    def get_length(self, link):
        req = self.skip_503("HEAD", link)#self.__session.head(link, headers=self.__headers)
        #req = self.__session.head(link, headers = self.__headers)
    ...
    def find_serie(self, name):
        ...
        if req.status_code==503:
            ...
            if req.status_code!=200:
                print(req.status_code)
                print("houdston we have a problem !!")
                sys.exit()
        

        #salvo la variabile session
        fw = open("session.pkl","wb")
        fw.write(pickle.dumps(self.__session))
        fw.close()
        '''
    ...
    def __get_episodes(self, name):
        url = self.__target+"?s="+name

        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:65.0) Gecko/20100101 Firefox/65.0'}
        req = self.__session.get(url, headers=headers)
        html = req.text
        ...
        if req.status_code==503:
            ...
            url0 = form["url"]
            params = form["params"]
            headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:65.0) Gecko/20100101 Firefox/65.0',"Referer":url}
            req = self.__session.get(url0, headers=headers, params=params, allow_redirects=True)
            html = req.text
        ...
        for item in soup.find_all("div",class_="result-item"):
            ...
            A = item.find_all("a")[1].text.lower()
            B = item.find("a").get("href")
            ratio = SequenceMatcher(None, A, name).ratio()
        ...
        print("apro:",link)
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:65.0) Gecko/20100101 Firefox/65.0'}
        req = self.__session.get(link, headers=headers)
        html = req.text
        soup = bs4.BeautifulSoup(html, 'html.parser')
        ...
        for ele in soup.find_all("div",class_="episodiotitle"):
            ...
            episodes.append({"name":name, "link":link, "s":sxe["s"], "e":sxe["e"]})

        
        return episodes

    def __get_episode_link(self, name, s, e):
        url = self.__target+"?s="+name
        ...
        for ele in soup.find_all("div",class_="episodiotitle"):
            t = self.__sxe(ele.a.get("href"))
            print(t)
            if t["s"]==s and t["e"]==e:
                return ele.a.get("href")
        return None



    def __get_src_from_link_with_selenium(self, link):
        driver = webdriver.Firefox()
        driver.install_addon(os.getcwd()+"\\ublock.xpi")
        driver.get(link)
    ...
    def next_episode(self):
        if self.__num<len(self.__episodes):
            src = self.__get_src_from_link_with_selenium(self.__episodes[self.__num+1]["link"])
            self.__num+=1
            return src
        else:
            return None
    ...
    def get_episode_by_name(self, serie, name):
        self.__episodes = self.__get_episodes(serie)
        link = None
        t = 0

        c=0
        for ele in self.__episodes:
            ratio = SequenceMatcher(None, ele["name"], name).ratio()
        ...
        src = self.__get_src_from_link_with_selenium(link)

        return src

    def get_episode_by_sxe(self, serie, s, e):
        self.__episodes = self.__get_episodes(serie)
        link = None
        ...
        for ele in self.__episodes:
            print(ele["s"],ele["e"])
            if ele["s"] == int(s) and ele["e"] == int(e):
                link = ele["link"]
                self.__num = c
                break
            c+=1
